...
def test_shellfish_north_rw_lot_chords():
    """
    The Lot 29/28/27 fronts are the Shellfish Dr N R/W curve (CL R=167.95' + 30' = 197.95'), not Keel Dr.
    Printed chords 51.68' / 68.75' / 59.50' give Δ 15°00' + 20°00' + 17°17'10" = CL Δ 52°17'10",
    and the chord bearings are tangent-consistent (N35°18'20"E + Δ/2 steps).
    """
    r = 197.95
    deltas = [2.0 * math.degrees(math.asin(c / (2.0 * r))) for c in (51.68, 68.75, 59.50)]
    assert abs(deltas[0] - 15.0) < 0.01
    assert abs(deltas[1] - 20.0) < 0.01
    assert abs(sum(deltas) - (52.0 + 17.0 / 60.0 + 10.0 / 3600.0)) < 0.02
...
def test_block16_all_33_lots_closed_and_precise():
...
def test_curve_and_line_tables_populated():
    ...
    assert c_table[6]["tag"] == "C7" and c_table[6]["radius"] == 197.95
...
if __name__ == "__main__":
    ...
    test_shellfish_north_rw_lot_chords()
    print("  [PASS] test_shellfish_north_rw_lot_chords")
    test_lot29_apex_convergence()
    print("  [PASS] test_lot29_apex_convergence")
    test_block16_all_33_lots_closed_and_precise()
    print("  [PASS] test_block16_all_33_lots_closed_and_precise (33/33 Lots Closed EXACT)")
