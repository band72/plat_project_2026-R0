...
ANCHORS = {
    ...
    "BLOCK_12": ("p5_sw", "F_BAYOU_MANGROVE_NE", []),
...
NOT_PLACED = {
    "BLOCK_14": "solver models a 24-lot double row with a cul-de-sac; the plat's Block 14 is the "
                "11-lot strip + Tract 'A' west of Mangrove Ave (Sheet 2) -- rebuild pending",
    "BLOCK_10": "no labelled fillet P.I. in the street network yet (corner is on the plat boundary) -- anchor pending",
    "BLOCK_6": "no solver yet (Sheet 2, east of Marina/Keel)",
    "BLOCK_7": "no solver yet (Sheet 2, Marina/Sands)",
    "BLOCK_8": "no solver yet (Sheet 2, Sands/Cape Horn)",
}
...
def main():
    ...
    for _street, pts in net.row_linework:
    ...
    dxf.text((tb[0] - 30, tb[1]), f"Full plat composed from street network + {len(placed)} block solvers "
             f"({lot_count} lots). Not placed: {', '.join(NOT_PLACED)}", 9.0, "TITLEBLOCK", halign=1, valign=2)
    ...
    ax.set_title("Beachwood Unit Two (PB 30 Pg 82/82A) - full plat, claude composition\n"
                 f"{len(placed)} blocks placed / {lot_count} lots; not yet placed: {', '.join(NOT_PLACED)}", fontsize=11)
    fig.tight_layout()
